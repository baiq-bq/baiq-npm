...
function validateTimedReadingMinMaxTime(
  time:
    | {
...
function createComponentSchema<T extends z.ZodRawShape>(type: string, props: T) {
...
const activitySchemas = activitySchemaPairs.map(
  ([type, pair]) => [type, pair.full] as const
) satisfies readonly (readonly [string, ZodType])[];
...
] as const satisfies readonly (readonly [string, ZodType])[];

/** Tuple member type for pairing activity type string with its Zod schema (for `z.infer` / narrowing). */
export type ComponentSchemaEntry = (typeof schemas)[number];

export type ComponentType = ComponentSchemaEntry[0];

export type ComponentSchemaMap = {
  [Entry in ComponentSchemaEntry as Entry[0]]: Entry[1];
};

export type ActivityPayloadByType = {
  [Entry in ComponentSchemaEntry as Entry[0]]: z.infer<Entry[1]>;
};
